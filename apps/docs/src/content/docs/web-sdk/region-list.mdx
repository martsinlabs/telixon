---
title: RegionList
description: The headless widget that produces a region picker's rows.
---

The headless widget that produces a region picker's rows. It emits a
[`RegionListState`](#regionliststate) snapshot on every change. Each change recomputes the
...
| `searchFn`         | `RegionSearchFn<T>`             | Custom search predicate; the default is described under [Search matching](#search-matching).                           |
...
Platforms without flag emoji, notably Windows, render the two letters. The
[flag sprite](/web-sdk/flags/) shows a flag on every platform.
...
The option for a region from the base set, regardless of the filters and the query. It is the same
object that appears in `options` and follows `localize` and `refresh`. A code the engine does not
know returns `undefined`.
