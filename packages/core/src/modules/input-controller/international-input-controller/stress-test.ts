...
import { createInternationalInputController } from './index';
...
function createController(suite: BenchmarkSuite) {
  return createInternationalInputController({
    defaultCountry: suite.country,
...
function createBenchmarkCases(suite: BenchmarkSuite): BenchmarkCase[] {
  return [
    {
      scenario: 'insert national digits',
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < SUITES.length; i++) {
    ...
    printResults('InternationalInputController', suite, results);
