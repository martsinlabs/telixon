import { CountryId, getCountryIndex } from '@telixon/core/engine';
...
export function getCountry(resolved: ResolvedPhoneNumber): CountryId | null {
  const { profileRef, callingCodeState, nationalDigits } = resolved;
  if (!profileRef) return null;

  const countryIndex: number = getCountryIndex(getResourceProvider().countryScopeLayer, profileRef.stateCountryIndex);
